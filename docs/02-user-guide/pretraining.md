...
| SpecForge | `framework: specforge` | `specforge_mode: capture`, or `train` with required `specforge_train_mode: offline` / `online` (Mooncake + SGLang), on a ROCm SGLang Primus image. See [SpecForge on Primus](../../examples/specforge/README.md). |

...
| SpecForge | `framework: specforge` | `specforge_mode: capture`, or `train` with required `specforge_train_mode: offline` / `online` (Mooncake + SGLang), on a ROCm SGLang Primus image. YAML / env / CLI: [SpecForge on Primus](../../examples/specforge/README.md#configuration-reference). |

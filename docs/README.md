...
- [SpecForge (capture / offline train / 2-node online train)](../examples/specforge/README.md): `primus-cli` entrypoint and YAML / env / CLI reference; `specforge_mode` is `train` or `capture`, and train requires `specforge_train_mode: offline` or `online`

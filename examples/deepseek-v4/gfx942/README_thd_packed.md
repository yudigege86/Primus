...
It is `TURBO_USE_GROUPED_MLP` that matters, not the yaml key: `run_deepseek_v4.sh` passes
`--use_turbo_grouped_gemm "$TURBO_USE_GROUPED_MLP"` on the command line, overriding whatever
the config says. Set both back on an image with a newer `primus_turbo`; CI builds it from
source rather than using the one in the image.

...
def _moe_unpermute_index_map_backward_fake(
    ...
    prob_grad = torch.empty((num_tokens, topK), dtype=torch.float32, device=unpermuted_act_grad.device)
    return act_grad, prob_grad
...
def _moe_permute_mask_map_forward_fake(  # pylint: disable=unused-argument
    ...
    fake_row_id_map = torch.empty((num_tokens, num_experts * 2 + 1), dtype=torch.int32, device=inp.device)
    if probs is not None:
        fake_permuted_probs = (

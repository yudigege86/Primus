...
def sparse_mla_bwd_v4_gluon_v2(q, kv, o, do, topk_indices, lse, attn_sink=None, kv_lora_rank=512, scale=None):
    ...
    # reload passes + repeated CSR builds. For high head counts (H>=128) the dq RMW volume
    # is large, so a single chunk over the whole topk (bounded for memory) is a big win at
    # pro cr4 (mirrors the triton bwd). Low head counts keep the 256 cap.
    if num_heads >= 128:

...
"""
Wan2.2 VAE (vendored from official Wan2.2 repo).

Source: `Wan2.2/wan/modules/vae2_2.py`

Minimal changes:
- add small `to()/eval()/train()` helpers on the wrapper to fit Wan trainer usage
  (HF trainer sometimes calls `model.vae.to(dtype=...)`).
"""
...
class WanVAE_(nn.Module):
    # NOTE: Keep this identical to upstream for numerical parity.
    def __init__(
        self,
        dim=160,
        dec_dim=256,
...
class Wan2_2_VAE:
    def __init__(
        ...
        if vae_pth is None:
            raise ValueError("Wan2_2_VAE requires `vae_pth` (checkpoint path).")
        self.model = (
            _video_vae(
    ...
    def encode(self, videos: List[torch.Tensor]):
        try:
            ...
            with amp.autocast(dtype=self.dtype):
                # Avoid forcing fp32 outputs: it increases memory and slows training.
                # Keep outputs in the model/autocast dtype (typically bf16).
    ...
    def decode(self, zs: List[torch.Tensor]):
        try:
            ...
            with amp.autocast(dtype=self.dtype):
                return [self.model.decode(u.unsqueeze(0), self.scale).clamp_(-1, 1).squeeze(0) for u in zs]
        except TypeError as e:
            logging.info(e)
            return None

...
from primus.backends.diffusion.utils.vision_process import strip_file_uri
...
class WanVideoDataset(BaseDataset):
    ...
    def load_video_decord(
        ...
        if isinstance(video_path, BytesIO):
            ...
            vr = _get_decord_vr(strip_file_uri(video_path[0]), num_threads=num_threads)
        elif isinstance(video_path, str):
            vr = _get_decord_vr(strip_file_uri(video_path), num_threads=num_threads)
        else:
